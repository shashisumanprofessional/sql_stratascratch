...
--e.salary > (SELECT AVG(salary) FROM employee WHERE department = e.department) ensures only employees earning above their department average are returned.
SELECT e.department, e.first_name, e.salary FROM employee e 
...
SELECT 
    ...
    (SELECT AVG(salary) 
     FROM employee 
     WHERE department = e.department) AS avg_salary
FROM employee e;
...
select s.department, e.first_name, e.salary, s.salary
from employee e inner join employee s
on e.first_name =s.first_name 
where s.salary = (select avg(salary) , department from employee 
group by department 
);
...
select department, first_name, salary, avg(salary) as avg_salary
from employee 
group by department, first_name, salary;
